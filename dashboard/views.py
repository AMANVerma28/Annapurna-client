...
def index(request):
    data = coreapi.Client()
    schema = data.get("https://itsproject-server.herokuapp.com/household/")
    with open('static/json/household.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("https://itsproject-server.herokuapp.com/farm/")
    with open('static/json/farm.json', 'w') as outfile:
	    json.dump(schema, outfile)
    schema = data.get("https://itsproject-server.herokuapp.com/well/")
    with open('static/json/well.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("https://itsproject-server.herokuapp.com/member/")
    with open('static/json/member.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("https://itsproject-server.herokuapp.com/season/")
    ...
    with open('static/json/storage.json', 'w') as outfile:
        json.dump(schema, outfile)
    schema = data.get("https://itsproject-server.herokuapp.com/yield/")
